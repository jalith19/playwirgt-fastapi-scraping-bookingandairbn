...
class MongoDB:
    ...
    async def save_many(self, collection_name: str, data: List[Dict], deduplicate_field: str = None):
        """Guardar múltiples documentos en una colección"""
        if not data:
            return 0
        ...
        if deduplicate_field:
            # Eliminar duplicados existentes antes de insertar
            ids_to_remove = []
            for item in data:
                if deduplicate_field in item:
                    existing = await collection.find_one({deduplicate_field: item[deduplicate_field]})
                    if existing:
    ...
    async def get_all(self, collection_name: str, limit: int = 100, skip: int = 0) -> List[Dict]:
    ...
    async def get_by_field(self, collection_name: str, field: str, value: Any, limit: int = 100) -> List[Dict]:
        """Buscar documentos por un campo específico"""
        collection = self.db[collection_name]
        cursor = collection.find({field: value}).limit(limit)
        data = await cursor.to_list(length=limit)
        for item in data:
            item["_id"] = str(item["_id"])
        return data
